      ...
      <div className="w-full max-w-sm rounded-2xl overflow-hidden"
        ...
        <div className="p-5 max-h-[80vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-white/70">뉴스 피드 관리</span>
            <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center text-white/30 hover:text-white/70 hover:bg-white/[0.06] transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>

          {/* 기본 피드 */}
          <p className="text-[10px] text-white/25 uppercase tracking-widest mb-2">기본 피드</p>
              ...
              return (
                <button key={f.name} onClick={() => onToggleDefault(f.name)}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl border transition-all text-left ${
                    on ? "bg-amber-400/10 border-amber-400/30" : "bg-white/[0.03] border-white/[0.05]"
                  }`}>
                  <Rss className={`w-3.5 h-3.5 flex-shrink-0 ${on ? "text-amber-400/70" : "text-white/20"}`} />
                  <span className={`text-xs flex-1 ${on ? "text-amber-200" : "text-white/25 line-through"}`}>{f.name}</span>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded ${on ? "bg-amber-400/15 text-amber-300/70" : "bg-white/[0.05] text-white/20"}`}>{f.category}</span>
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${on ? "bg-amber-400/70" : "bg-white/10"}`} />
                </button>
              );
            })}
          </div>
          ...
          {customFeeds.length > 0 && (
            <>
              <p className="text-[10px] text-white/25 uppercase tracking-widest mb-2">내 피드</p>
              <div className="space-y-1 mb-4">
                {customFeeds.map((f) => (
                  <div key={f.id} className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.06] group">
          ...
          {/* 피드 추가 */}
          <p className="text-[10px] text-white/25 uppercase tracking-widest mb-2">피드 추가</p>
          <div className="space-y-2">
            <input type="text" placeholder="피드 이름" value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="w-full bg-white/[0.04] text-[#f0ead6] text-sm rounded-xl px-3 py-2 outline-none focus:ring-1 focus:ring-amber-400/40 placeholder-white/20 border border-white/[0.06]" />
            <input type="text" placeholder="RSS URL" value={form.url}
              onChange={(e) => setForm({ ...form, url: e.target.value })}
              className="w-full bg-white/[0.04] text-[#f0ead6] text-sm rounded-xl px-3 py-2 outline-none focus:ring-1 focus:ring-amber-400/40 placeholder-white/20 border border-white/[0.06]" />
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
              className="w-full bg-white/[0.04] text-white/60 text-sm rounded-xl px-3 py-2 outline-none focus:ring-1 focus:ring-amber-400/40 border border-white/[0.06]">
...
export default function NewsWidget() {
  ...
  return (
    <>
      <div className="relative overflow-hidden bg-white/[0.05] backdrop-blur-2xl rounded-2xl p-4 sm:p-6 border border-white/[0.08] shadow-[0_4px_30px_rgba(0,0,0,0.25)] hover:border-orange-400/20 hover:shadow-[0_0_40px_rgba(249,115,22,0.10)] transition-all">
          ...
          <div className="space-y-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="animate-pulse flex gap-2.5">
                <div className="w-14 h-10 bg-white/[0.05] rounded-lg flex-shrink-0" />
            ...
            {filtered.map((item, i) => (
              <a key={i} href={item.link} target="_blank" rel="noopener noreferrer"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs sm:text-sm text-white/60 group-hover:text-white/90 line-clamp-2 leading-snug transition-colors">{item.title}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[10px] text-amber-400/50">{item.source}</span>
                    {item.pubDate && <span className="text-[10px] text-white/20">{timeAgo(item.pubDate)}</span>}
                  </div>
                </div>
